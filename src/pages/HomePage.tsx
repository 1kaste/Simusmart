...
const HeroSection: React.FC<{ settings: StoreSettings, onExploreClick: () => void }> = ({ settings, onExploreClick }) => {
  ...
  return (
    <section 
      ...
      style={heroStyle}
    >
      <div className="container mx-auto flex flex-col items-center justify-center px-4 py-20 text-center sm:px-6 lg:px-8 lg:py-32 bg-black/50">
        <h1 
          className="text-3xl font-extrabold tracking-tight sm:text-5xl md:text-6xl"
          style={{ color: settings.heroTextColor }}
        >
          {settings.heroTitle}
        </h1>
        <p 
          className="mt-4 max-w-2xl text-md sm:text-lg"
...
const CategoriesSection: React.FC = () => {
    ...
    return (
        <section className="py-12 sm:py-20">
            ...
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-center text-primary-dark dark:text-white">Shop by Category</h2>
...
const LatestArrivalsSection = React.forwardRef<HTMLElement>((props, ref) => {
    ...
    return (
        <section ref={ref} className="py-12 sm:py-20 bg-gray-50 dark:bg-primary-dark">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-center text-primary-dark dark:text-white">Latest Arrivals</h2>
            <div className="relative mt-10">
                <div className="flex space-x-4 md:space-x-6 overflow-x-auto pb-4 -mx-4 px-4 snap-x snap-mandatory">
                {products.slice(0, 8).map(prod => <ProductCard key={prod.id} product={prod} className="flex-shrink-0 w-60 sm:w-64 snap-start" />)}
                </div>
            </div>
            </div>
        </section>
